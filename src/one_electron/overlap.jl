...
hyp1f1=sp.hyp1f1
fact2=sp.factorial2
function Expansion_coeff(i::Float64, j::Float64, t::Int64, Qx::Float64, a::Float64, b::Float64)
    """
    ...
    elseif j == 0
        # for the decrement index i
        return (1.0 / (2 * p)) * Expansion_coeff(i - 1, j, t - 1, Qx, a, b) -((q * Qx / a) * Expansion_coeff(i - 1, j, t, Qx, a, b)) +((t + 1) * Expansion_coeff(i - 1, j, t + 1, Qx, a, b))
    else
        # for the decrement index j
        return (1 / (2 * p)) * Expansion_coeff(i, j - 1, t - 1, Qx, a, b) +
               (q * Qx / b) * Expansion_coeff(i, j - 1, t, Qx, a, b) +
               (t + 1) * Expansion_coeff(i, j - 1, t + 1, Qx, a, b)
    end
end
...
function overlap(a::Float64, lmn1::Vector{Float64}, A::Vector{Float64}, b::Float64, lmn2::Vector{Float64}, B::Vector{Float64})
    ...
    l1, m1, n1 = lmn1
    l2, m2, n2 = lmn2 
    S1 = Expansion_coeff(l1, l2, 0, A[1] - B[1], a, b) # X
    S2 = Expansion_coeff(m1, m2, 0, A[2] - B[2], a, b) # Y
    S3 = Expansion_coeff(n1, n2, 0, A[3] - B[3], a, b) # Z
